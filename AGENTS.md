...
- **`configuration.actionGroups`** describes the unified action model. Each group can target specific environments and nest sequential `actions`. Supported `type` values: `terraform`, `bicep`, `powershell`. Actions inherit the group type and must not declare their own. Token replacement defaults per tech can be overridden via `tokenTargetPatterns`, `tokenPrefix`, `tokenSuffix`. Use `dependsOn` to order actionGroups when sequential execution is required. PowerShell groups support an optional `kind` (currently `pester`) that turns on automatic NUnit publishing using the convention `TestResults/<actionGroup>_<action>.xml`; override the path/title per action via `testResultsFiles` / `testRunTitle` when needed.
- **Variable overrides** let action groups inject ephemeral variables into deployments or test jobs. Set `variableOverridesEnabled: true` and provide a `variableOverrides` object (plus an optional `testsDeploymentSuffixToken`) to load `templates/variables/include-overrides.yml`. The helper surfaces dynamic suffix values (`runSuffix`, `deploymentVersion`) and any arbitrary keys so scripts and parameter files can toggle behaviours like dynamic deployment versions or test-specific filters.
- **`configuration.pipelineType`** (string, optional) appends a suffix to Azure DevOps Environment names (e.g. `DEV-AUTO`). Set this when a pipeline variant should land in a dedicated environment scope such as automated test lanes; the value flows through validation, review, and deploy stages.
...
- **Variables** load compile-time via `templates/variables/include.yml`. Global defaults live under `configuration.variables`, per-environment overrides under `env.variables`. Flags (`includeCommon`, `includeRegion`, `includeEnv`, `includeEnvRegion`) prevent missing-file failures when layers are unused.
